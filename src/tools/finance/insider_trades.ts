...
import { z } from 'zod';
import { fetchInsiderTrades } from './free-data.js';
...
export const getInsiderTrades = new DynamicStructuredTool({
  ...
  description: `Retrieves insider trading transactions for a given company ticker. Insider trades include purchases and sales of company stock by executives, directors, and other insiders. This data is normalized from free SEC-linked market data sources. Use filing_date filters to narrow down results by date range. Use the name parameter to filter by a specific insider.`,
  schema: InsiderTradesInputSchema,
  func: async (input) => {
    const { data, sourceUrls } = await fetchInsiderTrades({
